...
class ItemTask extends Component {
  ...
  render() {
    let { ordered, data, dataProject, users } = this.props;
      ...
      <tbody>
        {
          ...
          ? (
            ordered.map( (e, i) => {
              let item = data[e];
              if(!item || item.projectId !== dataProject.id) return null;
              ...
              return (
                <tr key={i}>
                  ...
                  <td className="text-center">
                    <Link to={`/users/report/${item.memberId}`}>
                      {nUser}
                    </Link>
                  </td>

                  <td className="text-center">
                    {!!item.begin ? convertTime(item.begin) : ""}
                  </td>

                  <td className="text-center">
                    {!!item.end ? convertTime(item.end) : ""}
                  </td>
