...
export const deleteFinished = (id):Action => {
  return {
    type: constant.DELETE_FINISHED,
    payload: id
  };
};
...
export const fetchAll = (filter?, skip?, limit?, where?) => {
  return (dispatch: (action: Action) =>void) => {
    ...
    api.cateTask.get(filter, skip, limit, where)
      .then(res => {
...
export const create = (data) => {
  return (dispatch: (action) => void) => {
    dispatch(fetchStarted());
    return api.cateTask.create(data)
      .then(obj => {
        if(obj.error)
...
export const updateById = (id, data) => { 
  return (dispatch: (action) => void) => {
    dispatch(fetchStarted());
    return api.cateTask.updateById(id, data)
      .then(obj => {
        if(!!obj.data)
          dispatch(fetchFinished([obj.data]))
...
export const deleteById = (id) => {
  return (dispatch: (action) => void) => {
    dispatch(fetchStarted());
    return api.cateTask.del(id)
      .then(obj => {
        if(!!obj.error) dispatch(fetchFailed(obj.error));
        dispatch(deleteFinished(id))
        return obj
      });
  }
}